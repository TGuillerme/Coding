...
#<Chain name>
#Notes on TEMsim_v2.6_8c.sh options:
...
#
#----
#guillert(at)tcd.ie - 22/01/2014
##########################
#Requirements:
#-Shell script Matsim_v4.3.1.sh
#-R 3.0.1
#-R package "ape"
#-R package "phyclust"
...
sed 's/MM/M'"${Chain}"'/g' Matsim_v4.3.1.sh | sed 's/Simulation_@.log/Simulation_'"${Chain}"'.log/g' > ${TotalSp}t_${TotalChar}c_${Model}_${Method}_${Chain}/Matsim_${Chain}.sh
...
for f in *.phylip ; do echo $f ; seqConverter.pl -d${f} -ip -on -ri; done ;
...
#IF STATEMENT = 'datatype = protein' - IF-BLOCK a STOP
...
#IF STATEMENT = 'datatype = nucleotide' - IF-BLOCK b STOP
...
#IF STATEMENT = 'datatype = DNA' - IF-BLOCK b STOP
...
for f in *.nex.tmp ; do prefix=$(basename $f .nex.tmp) ; echo converting ${prefix}; sed 's/\([a-z]\)\([a-z]\)\([0-9]\)\([0-9]\)\([0-9]\)[[:space:]]\([0-9]\)/\1\2\3\4\5\6	/g' $f > ${prefix}.nex ; done ;
rm *.nex.tmp
...
#[ML method, fully available in TEMsim_v2.6]


#IF STATEMENT = 'Chosen method: ML' - IF-BLOCK 1 STOP



#IF STATEMENT = 'Chosen method: !ML' - IF-BLOCK 2 START
...
#IF STATEMENT = Chosen method: !ML - IF-BLOCK 2 STOP
fi ;
...
#echo "" > ${Chain}_tasks.config
#for ((i=0 , k=1 ; i<=192 , k<=25 ; i+=8 , k+=1))
#do echo "$i sh ${Chain}_$k.task" >> ${Chain}_tasks.config
#done
...
#srun --multi-prog ${Chain}_tasks.config" | sed 's/\/\#!/\#!/g' > ${Chain}.job.sh
